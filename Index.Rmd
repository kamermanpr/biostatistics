...
```
- Change _proxy.server.com_ to the address of your proxy server (e.g., _proxyad.wits.ac.za_, _proxyss.wits.ac.za_)
...
If you want to clear your proxy settings (e.g., if you are working on a laptop that you use at home and at work), enter the following commands in the _terminal_ or _git bash_:
...
On OSX and Linux machines the home directory is usually your 'username' directory. On Windows machines the home directory is usually the the users 'Documents' directory.
...
Creating hidden _'dot'_ files like _.Renviron_ can be a nightmare in Windows, so here are some instructions on how to add the file from the command prompt (**Note:** You can also run these lines of code in your OSX or Linux terminal):
...
4. Enter the following lines one at a time in the command prompt editor (remember to change the proxyserver and port to your settings):
