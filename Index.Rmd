...
If you are using computers in the School of Physiology computer lab, the computers may not have the software installed, or some/all of the software may be outdated. If so, please install/update the software. Because computers in the computer laboratory have multiple users, please note the following:

**Do not** set the user configuration using the global switch before initializing a new git repository:
...
Rather navigate to the folder you want to initiate as a git repositiory and set the configurations as a local user: 
...
```
- Change _proxy.server.com_ to the URL of your proxy server (e.g., _proxyad.wits.ac.za_, _proxyss.wits.ac.za_)

- Change _8080_ to the proxy port configured on your proxy server (e.g., _80_)

Try the settings above, but if your proxy screams out for a username and password, add the following _(not a great option on a computer used by multiple users)_:

```
$ git config --global http.proxy
http://proxyusername:proxypassword@proxyserver.com:8080/

$ git config --global https.proxy
https://proxyusername:proxypasswordproxy.server.com:8080/
...
If you want to clear your proxy settings (e.g., if you are working on a laptop that you use at home and at work), enter the following command in the _terminal_ or _git bash_:
...
On OSX and Linux machines, the home directory is usually your user directory. On Windows machines, the home directory is usually the the users 'Documents' directory.

**OSX and Linux R home directory (example):**
```
/User/username
```

**Windows R home directory (example):**
```
C:\Users\username\Documents
```

If you cannot locate your home directory, enter the following in _R_:
```
path.expand("~")
```

The _.Renviron_ file should contain the proxy settings for your location. The file must be in your _R_ home directory before you start _R/RStudio_, but remember that this is a hidden file, so you will have to have your file view set to show hidden files if you want to see the file _in situ_. The following is an example of the text that should be in the _.Renviron_ file:
...
Creating hidden _'dot'_ files like _.Renviron_ can be a nightmare in Windows, so here are some on how to add the file from the command prompt (**Note:** You can also run these lines of code in your OSX or Linux terminal):

1. Open Windows File Explorer;

2. Navigate to your _R_ home directory (e.g., C:\User\username\Documents);

3. Hold down the _'Shift'_ button and _'Right-click'_ on the folder. Choose _'Open command window here'_ from the context menu.

4. Enter the following lines in the command prompt editor (remember to change the proxyserver and port to your settings):

```
echo http_proxy=http://proxyserver.com:8080 > .Renviron
echo http_proxy_user=ask >> .Renviron
echo https_proxy=https://proxyserver.com:8080 >> .Renviron
echo https_proxy_user=ask >> .Renviron
```

The first line creates a file called _.Renviron_ with _http_proxy=http://proxyserver.com:8080_ as the first line of text. The subsequent lines append additional text to the preceeding line of text.

When you are outside of the work setting, either rename the _.Renviron_ file, move the file to another directory, or delete the file before opening _R/RStudio_. 
